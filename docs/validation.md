...
## 頁面偏好、紀念日與 OTA 確認 1.4.4（2026-09-26）

設定頁可拖曳或按上下按鈕調整六頁順序，預設系統資訊最後。紀念日顯示在時鐘頁下方，支援固定日期倒數／累計及每年重複；小字中文使用 Flash 字庫，不配置動態字型緩衝區。新增偏好與忽略版本使用獨立 NVS key，維持原 Wi-Fi／薪資設定 record 相容。

OTA 開機只檢查一次；確認時綁定版本，重新下載 metadata 若版本改變則重新詢問。預選稍後、不再提醒只針對一個版本，手動檢查可再次提示。更新畫面固定亮屏，進出彈窗先等待按鈕放開，避免翻頁／休眠手勢誤觸確認。

本機通過：核心 14,881,120 項、OTA 傳輸／版本／故障注入 283 項、NVS 130 項、JSON 設定 126 項、頁面／日期／更新政策 975 項。新增檢查包含非法頁序、舊設定預設值、偏好及忽略版本重啟保存、寫入失敗、閏年與跨年紀念日、非法 UTF-8、同版本不再提醒、手動再次提示、不同版本不得沿用確認、開機單次檢查，以及三種配色的整幀／strip 一致性與 buffer guard。

`tests/test_portal.cjs` 使用 Playwright 與 Chrome，在 390px 手機及 1024px 桌面尺寸驗證排序、桌面拖曳、恢復預設、名稱驗證、日期與重複選項儲存、既有密碼保留、無 JavaScript 錯誤及無水平溢出；已目視檢查設定頁與 LCD 排版。執行方式：安裝 `playwright` 後執行 `node tests/test_portal.cjs`，可用 `CHROME_PATH` 指定 Chrome。預覽位於 `.artifacts/host/portal-390.png`、`portal-1024.png` 與 `preferences-preview.png`。

兩款 PlatformIO 韌體皆需通過建置與 release image 檢查後才發布。尚無本次實板測試：請驗收按鍵切換時機、NVS 斷電、Wi-Fi 中斷、實際進度更新、下載失敗返回及 A/B 回滾。
